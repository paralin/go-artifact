...
func GenerateAPI(ctx context.Context, clientOutput, eventsOutput io.Writer) error {
	senderMap := make(map[MsgID]MsgSender)
	...
	fmt.Fprintf(clientOutput, "package artifact\n\nimport (\n")

	msgIds := getSortedMsgIDs()
	var requestFuncs []*generatedRequestFunc

	eventsImports := make(map[string]struct{})
	eventsImports["github.com/faceit/go-artifact/protocol"] = struct{}{}
	...
	clientImports["github.com/faceit/go-artifact/protocol"] = struct{}{}
	clientImports["github.com/faceit/go-artifact/events"] = struct{}{}

	// responseMsgs are messages that are known to be responses.
	responseMsgs := make(map[MsgID]struct{})
	eventHandlers := make(map[MsgID]*generatedEventHandler)
	...
	for _, f := range requestFuncs {
		fmt.Fprintf(clientOutput, "\n// %s\n", f.generateComment())
		fmt.Fprintf(clientOutput, "func (d *Artifact) %s(\n", f.methodName)
		...
		if respTyp != nil {
			...
			fmt.Fprintf(clientOutput, "\t\tctx,\n\t\tuint32(protocol.EGCDCGClientMessages_%s),\n", f.reqMsgID.String())
			fmt.Fprintf(clientOutput, "\t\treq,\n\t\tuint32(protocol.EGCDCGClientMessages_%s),\n", f.respMsgID.String())
			fmt.Fprintf(clientOutput, "\t\tresp,\n\t)\n")
		} else {
			fmt.Fprintf(clientOutput, "\td.write(uint32(protocol.EGCDCGClientMessages_%s), req)\n", f.reqMsgID.String())
		}
	...
	fmt.Fprintf(clientOutput, "func (d *Artifact) registerGeneratedHandlers() {\n")
	...
	for _, eventHandler := range eventHandlersOrdered {
		...
		fmt.Fprintf(eventsOutput, "func (e *%s) GetEventMsgID() protocol.EGCDCGClientMessages {\n", eventHandler.eventName)
		fmt.Fprintf(eventsOutput, "\treturn protocol.EGCDCGClientMessages_%s\n", eventHandler.msgID.String())
		fmt.Fprintf(eventsOutput, "}\n")
		...
		fmt.Fprintf(
			clientOutput,
			"\td.handlers[uint32(protocol.EGCDCGClientMessages_%s)] = d.getEventEmitter(func() events.Event {\n",
			eventHandler.msgID.String(),
		)
